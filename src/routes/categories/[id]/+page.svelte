...
<StickyNav>
  <div slot="left">
    <a class="button" href="{getLocale() === 'en' ? "/en" : "/"}"><Icon i="chevron" small rotate={180} /> {getLocale() === 'en' ? "back" : "retour"}</a>

    {#if data.pilier}
    <h4><a href="{getLocale() === 'en' ? "/en" : ""}/categories/{data.pilier.fields.id}">{data.pilier.fields.titre}</a></h4>
    {/if}
  </div>
</StickyNav>
...
<Hero media={data.categorie.fields.photo}>
  <section>
    ...
    <aside>
      ...
      <Filters categories={[data.categorie]} {checked} base={`/categories/${data.categorie.fields.id}`} on:update={event => checked = event.detail} />
      {/if}
    </aside>
  </section>
</Hero>
...
<section>
  ...
  <aside>
    {#if data.categorie.fields.sousCategories}
    <!-- <h5>Sous-catégories</h5> -->
    <Filters categories={data.categorie.fields.sousCategories} {checked} base={`/categories/${data.categorie.fields.id}`} on:update={event => checked = event.detail} />
    {/if}
  </aside>
</section>
{/if}
