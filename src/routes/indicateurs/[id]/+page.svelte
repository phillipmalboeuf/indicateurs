...
<StickyNav>
  <div slot="left">
    <a class="button" href="/"><Icon i="chevron" small rotate={180} /> {getLocale() === 'en' ? "back" : "retour"}</a>

    {#if data.pilier}
    <h4>
      <a style="color: {data.pilier.fields.couleur}" href="/categories/{data.pilier.fields.id}">{data.pilier.fields.titre}</a> – <a style="color: {data.indicateur.fields.categorie.fields.couleur}" href="/categories/{data.indicateur.fields.categorie.fields.id}">{data.indicateur.fields.categorie.fields.titre}</a>  
    </h4>
    {/if}
  </div>
</StickyNav>
...
<section class:full>
  ...
  <figure class:ex={$page.url.searchParams.has("export")}>
    {#if data.indicateur.fields.data}
    <div>
      <button on:click={() => full = !full}>{#if getLocale() === 'en'}{#if full}Half screen ↴{:else}Full screen ⤢{/if}{:else}{#if full}Moitié d'écran ↴{:else}Plein écran ⤢{/if}{/if}</button>
    </div>
    {#key alt}
    <Chart 
    ...
    {/key}
    {:else}
    <em class="empty">{getLocale() === 'en' ? "To be documented" : "À documenter"}</em>
    {/if}
  </figure>
</section>
{/key}
