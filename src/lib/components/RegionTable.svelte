<script lang="ts">
  ...
  export let click: MouseEventHandler<HTMLInputElement> = undefined
</script>
...
<table class="regions" class:click>
  ...
  <tr>
    ...
    <td style="--color: {regions[r]}">
      {#if click}<input onclick={click} bind:group={$region} type="checkbox" name={r} id={r} value={r} />{/if}
      <label for={r}><img src="/regions/{r.toLowerCase().replace('é', 'e')}.svg" alt="" /> {getLocale() === 'en' ? regionsEnglish[r] : r}</label>
    </td>
    {/each}
  </tr>
  {/each}
