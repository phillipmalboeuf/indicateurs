<script lang="ts">
  ...
  export let indicateurs: (Entry<TypeIndicateurSkeleton, "WITHOUT_UNRESOLVABLE_LINKS"> & { hidden?: boolean })[]
  let Component
...
<ul class:one={indicateurs.length === 1} class:ex={page.url.searchParams.has("export")} style="grid-template-columns: repeat({indicateurs.length < 4 ? indicateurs.length : 4}, 1fr);">
  ...
  <li>
    ...
    <svelte:component this={Component} locale={getLocale()} juri={indicateur.fields.id.startsWith('progres') && indicateur.fields.id.split('–')[1]} />
    {/if}
    {:else}
    <Card {indicateur} />
    {/if}
