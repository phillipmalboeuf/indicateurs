...
<path d="M72 32C72 29.2386 74.2386 27 77 27H99C101.761 27 104 29.2386 104 32V112H72V32Z" fill="#F1D268"/>
      ...
      <strong>{getLocale() === 'en' ? 'The Indicators' : 'Les Indicateurs'}</strong>
    </a>
  </div>
  <nav>
    ...
    <a class="locale" data-sveltekit-reload href={localizeHref(page.url.pathname, { locale: getLocale() === 'en' ? 'fr' : 'en' })} hreflang={getLocale() === 'en' ? 'fr' : 'en'}><strong>{getLocale() === 'en' ? "FR" : "EN"}</strong></a>
  </nav>
