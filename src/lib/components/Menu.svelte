...
<nav transition:fly={{ y: -100 }}>
  <button class="close" on:click={() => visible = false}><Icon i="close" /></button>
	...
	<ul on:click={() => visible = false}>
		<li><a href="/" class="logo"><strong>{getLocale() === 'en' ? 'The Indicators' : 'Les Indicateurs'}</strong></a></li>
    ...
    {/each}
		...
		<li>
			<a class="locale" data-sveltekit-reload href={deLocalizeUrl(page.url).pathname} hreflang={getLocale() === 'en' ? 'fr' : 'en'}><strong>{getLocale() === 'en' ? "Français" : "English"}</strong></a>
		</li>
	</ul>
</nav>
{/if}
